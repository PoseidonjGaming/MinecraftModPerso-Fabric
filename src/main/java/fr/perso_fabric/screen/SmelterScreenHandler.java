...
import fr.perso_fabric.inventory.Slot.ModSlot;
import fr.perso_fabric.inventory.Slot.ResultSlot;
...
public class SmelterScreenHandler extends ScreenHandler {
    ...
    public SmelterScreenHandler(int syncId, PlayerInventory playerInventory, Inventory inventory, PropertyDelegate delegate) {
        super(ModScreen.smelter_screen_handler, syncId);
        checkSize(inventory,4);
        this.inventory=inventory;
        inventory.onOpen(playerInventory.player);
        this.propertyDelegate=delegate;

        this.addSlot(new Slot(inventory,0,70,7));
        this.addSlot(new Slot(inventory,1,90,7));

        this.addSlot(new ResultSlot(inventory,2,80,62));

        this.addSlot(new ModSlot(inventory,3,54,43));
